...
app.get('/lapPenjualan', (req, res) => {
    if (req.session.role === 'admin') {
        ...
        console.log('Berhasil berada di halaman laporan stok barang');
        res.render('lapPenjualan', { title: 'Laporan Penjualan', penjualan: results });
    });
    } else {
        res.status(403).send('Forbidden');
    }
...
app.post('/penjualan/save', (req, res) => {
    ...
    const totalPembayaran = req.body.isMember === 'on' ? totalHarga : parseFloat(req.body.subtotal);
    ...
    conn.query(insertQuery, [values], (err, result) => {
        ...
        res.render('strukPenjualan', { data: strukData });
    });
});
